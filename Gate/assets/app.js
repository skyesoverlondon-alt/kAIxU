(function () {
  ...
  function brandEngine(value) {
    const raw = String(value || "");
    const normalized = raw.trim().toLowerCase();
    if (["openai", "anthropic", "gemini", "voyage"].includes(normalized)) return "Skyes Over London";
    return raw;
  }
  function maskProviderLabels(text) {
    return String(text || "").replace(/openai|anthropic|gemini|voyage/gi, "skyes-over-london");
  }
  ...
  function warnIfUnpriced(allowedProviders, allowedModelsObj) {
    try {
      ...
      // If allowed_models is an object like {"skyes-over-london":["kAIxU6.7-flash"]}
      if (allowedModelsObj && typeof allowedModelsObj === "object") {
        for (const [provider, models] of Object.entries(allowedModelsObj)) {
          if (!PRICED[provider]) {
            warnings.push(`Engine "${provider}" is not priced.`);
            continue;
          }
// ------------------------------
...
function mountPricedModelPicker(textareaSel, providersInputSel) {
  ...
  for (const p of providers) {
    ...
    opt.value = p;
    opt.textContent = "Skyes Over London";
  ...
  includeTxt.textContent = "Also add engine to allowed_providers";
  ...
  btnClearProv.textContent = "Remove engine from JSON";
  ...
  renderModels();
        ...
        for (const p of provs) {
          if (!PRICED[p]) warnings.push(`Engine "${p}" is not priced.`);
        }
      }
  ...
  async function loadKeys() {
    ...
    for (const k of (data.keys || [])) {
      ...
      tr.innerHTML = `
        ...
        <td>${k.allowed_providers ? escapeHtml((k.allowed_providers || []).map(brandEngine).join(",")) : "—"}</td>
        <td style="max-width:260px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${k.allowed_models ? escapeHtml(maskProviderLabels(JSON.stringify(k.allowed_models))) : "—"}</td>
    ...
    if (tbody) {
      ...
      for (const ev of (data.events || [])) {
        ...
        tr.innerHTML = `
          ...
          <td>${escapeHtml(brandEngine(ev.provider))}</td>
...
async function generatePushInvoice() {
  ...
  function renderMonitorRow(ev) {
    ...
    const provModel = brandEngine(ev.provider || "") + (ev.model ? ("/" + ev.model) : "");
