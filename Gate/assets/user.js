    ...
    if (numbersEl){
      if (!nums.length) numbersEl.innerHTML = "<div class='muted'>No voice numbers mapped to this customer yet.</div>";
        ...
        <div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06)">
          <div><b>${esc(n.phone_number)}</b> <span class="pill">${esc(brandEngine(n.provider))}</span> ${n.is_active ? "" : "<span class='pill danger'>disabled</span>"}</div>
          <div class="muted">LLM: ${esc(brandEngine(n.default_llm_provider))} · ${esc(n.default_llm_model)} · ${esc(n.locale)} · ${esc(n.timezone)}</div>
        </div>
      `).join("");
    }
    if (pricingEl){
