...
export default wrap(async (req) => {
  ...
  const providerInput = (body.provider || "").toString().trim().toLowerCase();
  const provider = providerInput === "skyes-over-london" ? "gemini" : providerInput;
  const modelInput = (body.model || "").toString().trim();
  const modelAliases = {
    "kaixu-embed-001": "embedding-001",
    "kaixu-embed-004": "text-embedding-004"
  };
  const model = modelAliases[modelInput] || modelInput;
  ...
  if (!provider) return badRequest("Missing engine identifier", cors);
  if (!model)    return badRequest("Missing model identifier", cors);
  ...
  try {
    if (provider === "gemini") {
      ...
      return badRequest("Embedding engine not supported by this deployment.", cors);
    }
  } catch (e) {
    return json(e?.status || 500, { error: e?.message || "Engine error" }, cors);
  }
